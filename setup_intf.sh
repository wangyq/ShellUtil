...
#INTFS=(lo eth1=1.1.1.4/24,1.1.2.1/24,2.2.2.2/24 )
...
#-----------------------------------------------
# Clear all ip addresses in specific interfaces.
clear()
{
        for f in ${INTFS[*]}; do
                inf=${f%=*}
                addr=${f#*=}
                echo Clear all addresses in inf=$inf, addr=$addr ......
                #if [ -n "$inf" -a -n "$addr" ] ; then
                if [ -n "$inf" ] ; then
...
case $1 in
	...
	show)
		show ;;
        *)
        echo "Usage: $0 start | stop | restart | clear | show"
esac
}
